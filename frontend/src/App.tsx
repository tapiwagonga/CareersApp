...
function App() {
  ...
  // --- 2. SMART PROFILE SYNC ---
  useEffect(() => {
    if (profile && user) {
      
      const dbName = profile.name || user.email?.split('@')[0] || "Guest";
      const dbRole = profile.role || "Career Explorer";
      const dbConfig = (profile.avatar_config && Object.keys(profile.avatar_config).length > 0) 
        ? profile.avatar_config 
        : null;

      const local = state.userProfile;
      const isEmpty = !local;
      const isDifferentUser = local && local.email !== user.email;
  ...
  const handleProfileUpdate = async (newName: string, newRole: string, newEmail: string) => {
    console.log("👤 Syncing Profile to DB...", { newName, newRole }); 
  ...
  const handleAvatarSave = async (newConfig: any, newName: string) => {
    const currentRole = state.userProfile?.target_role || profile?.role || "Career Explorer";
    const currentEmail = state.userProfile?.email || user?.email || "";
  ...
  const showRoadmapNav = viewMode === "roadmap" 
    && state.currentStep !== AppStep.PROFILE_SETUP 
    && state.currentStep !== AppStep.AUTH;

  const activeProfile = {
      name: state.userProfile?.name || profile?.name || user?.email?.split('@')[0] || "Guest",
      role: state.userProfile?.target_role || profile?.role || "Career Explorer",
      email: state.userProfile?.email || user?.email || "guest@example.com",
      avatar_config: state.userProfile?.avatar_config || profile?.avatar_config
  };
  ...
  return (
    ...
    <header className="fixed top-0 left-0 right-0 z-40 bg-white/80 backdrop-blur-xl border-b border-gray-100 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        {state.currentStep !== AppStep.AUTH && (
          <div className="hidden md:flex bg-gray-100 p-1 rounded-full relative">
             <button onClick={() => setViewMode("roadmap")} className={`relative px-6 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-2 z-10 ${viewMode === "roadmap" ? "text-white" : "text-gray-500 hover:text-gray-900"}`}>
                <Map size={14} /> Roadmap
             </button>
             <button onClick={() => setViewMode("interview")} className={`relative px-6 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-2 z-10 ${viewMode === "interview" ? "text-white" : "text-gray-500 hover:text-gray-900"}`}>
                <MessageSquare size={14} /> Coach
             </button>
             <motion.div className="absolute top-1 left-1 bottom-1 w-[calc(50%-4px)] bg-black rounded-full shadow-md z-0" animate={{ x: viewMode === "roadmap" ? 0 : "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }} />
          </div>
        )}
        
        {/* PROFILE / AUTH / STATUS BUTTONS */}
        <div className="flex items-center gap-4">
            
            {/* Connection Status Indicator */}
            {isConnected !== null && (
              <div title={isConnected ? "Database Connected" : "Database Offline"} className={`p-2 rounded-full ${isConnected ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}>
            ...
            {user ? (
              ...
              >
                {activeProfile.avatar_config ? (
                  <Avatar className="w-full h-full" {...activeProfile.avatar_config} />
                ) : (
                  <User size={18} />
                )}
              </div>
                    ...
                    {ROADMAP_STEPS.map((s, idx) => {
                        ...
                        return (
                            <div key={s.id} className="flex items-center">
                                <div onClick={() => canNavigate && actions.goToStep(s.step)} className={`flex items-center gap-2 px-2 md:px-3 py-1.5 rounded-full transition-all duration-300 ${canNavigate ? "cursor-pointer hover:bg-gray-50" : "cursor-not-allowed opacity-30"}`}>
                                    <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center text-[10px] font-bold border transition-all ${isCurrent ? "border-black bg-black text-white scale-100 shadow-lg" : isCompleted ? "border-black bg-white text-black" : "border-gray-200 text-gray-300"}`}>
              ...
              {state.currentStep === AppStep.DASHBOARD && state.analysis && (
                <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <Dashboard 
                    ...
                    userId={activeProfile.email !== "guest@example.com" ? activeProfile.email : undefined}
                  />
                </motion.div>
              )}
            </>
          ...
          {viewMode === "interview" && (
             <motion.div key="interview" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
                {(state.targetJob || interviewContext) ? (
                  ...
                  <InterviewSetup 
                    onStart={(role, company, jd) => setInterviewContext({ role, company, jd })} 
                    onCancel={() => setViewMode("roadmap")}
                  />
                )}
             </motion.div>
          )}
      ...
      <AnimatePresence>
        {showProfile && (user || state.userProfile) && (
          <UserProfileView 
            user={activeProfile as any}
            stats={{ 
      ...
      {isEditingAvatar && (
        <div className="fixed inset-0 z-[60] bg-white">
          <AvatarBuilder 
             initialConfig={activeProfile.avatar_config}
