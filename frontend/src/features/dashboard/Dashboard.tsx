...
import {
  Play, BookOpen, Mic, Layout, Zap, Clock, ShieldAlert, CheckCircle2,
  ChevronRight, Calendar, Trophy, Lock, MapPin, Check, Star,
  Flame, TrendingUp, AlertCircle, Sparkles, Image as ImageIcon,
  Maximize2, Flag, Timer, Home, Terminal, Github, Loader2,
  Menu, X, Briefcase, UserCheck
} from "lucide-react";
...
const isTaskDone = (t: any) =>
  t?.status === "Completed" || t?.status === "Done" || Boolean(t?.is_completed) || Boolean((t as any)?.is_completed);
  ...
  return (
    <div className={`p-4 rounded-xl border ${status.bg} ${status.border}`}>
      ...
      <div className="flex justify-between items-end text-xs font-medium text-gray-500 mt-2">
        <span>Day {daysElapsed} of {totalDaysEst}</span>
      ...
      <div className="relative h-1.5 bg-gray-200 rounded-full mt-2 overflow-hidden">
        ...
        <motion.div
          ...
          className={`absolute top-0 left-0 h-full ${
            diff < -10 ? "bg-rose-500" : diff > 5 ? "bg-amber-400" : "bg-emerald-500"
          }`}
        />
      </div>
    </div>
    ...
    <div className="relative py-10 px-4">
      ...
      <div className="space-y-8 relative z-10">
        {phases.map((phase, index) => {
          ...
          return (
            <div key={phase.week_number || index} className="relative pl-16 group">
              <button
                type="button"
                onClick={() => {
                ...
                disabled={isLocked}
                className={`
                  absolute left-5 -translate-x-1/2 w-10 h-10 rounded-full border-4 flex items-center justify-center transition-all duration-300 z-20
              ...
              <button
                ...
                className={`
                  ...
                  ${isActive ? "bg-white border-black shadow-md translate-x-1" : isLocked ? "bg-transparent border-transparent opacity-50 cursor-not-allowed" : "bg-white border-gray-100 hover:border-gray-200"}
                `}
              >
                ...
                <h3 className={`font-bold text-sm leading-tight ${isActive ? "text-gray-900" : "text-gray-500"}`}>
                  {phase.focus_area}
                </h3>
              </button>
            </div>
...
const BossBattleCard = ({
  ...
  return (
    <motion.div
      ...
      className={`
        group relative w-full rounded-2xl border-2 transition-all overflow-hidden
    ...
    >
      ...
      <div className="p-6 md:p-8">
        <div className="flex justify-between items-start mb-6">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <h4 className="text-xl font-bold text-white tracking-tight">
                  {(task.title || "").replace(/Build:|Project:/i, "").trim()}
                </h4>
                {isCompleted && (
          ...
          <button
            type="button"
            className={`p-2 rounded-full hover:bg-white/5 transition-colors text-gray-500 ${expanded ? "rotate-90" : ""}`}
          >
            ...
            {expanded && requirements.length > 0 && (
              ...
              >
                <div className="pt-4 border-t border-white/5">
                  <div className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-3">
                    Acceptance Criteria
                  </div>
                  <div className="grid gap-2">
                    {requirements.map((req, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-3 p-3 rounded-lg bg-black/20 border border-white/5 text-sm text-gray-400 font-mono"
                      >
...
const MissionCard = ({
  ...
  if (isProject) {
    return <BossBattleCard task={task} isCompleted={isCompleted} onToggle={onToggle} />;
  }

  const Icon = TYPE_ICONS[type] || TYPE_ICONS.doc;
  const colorClass = TYPE_COLORS[type] || TYPE_COLORS.doc;
  const isMedia = type === "video" || type === "Watch";
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className={`
        group relative w-full p-5 rounded-2xl border-2 transition-all cursor-pointer overflow-hidden
        ${isCompleted ? "bg-gray-50 border-gray-100 opacity-70 grayscale" : "bg-white border-gray-100 hover:border-black hover:shadow-lg"}
      `}
      onClick={isMedia ? onPlay : () => onToggle()}
    >
      <div className="flex gap-4 relative z-10">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border ${colorClass} ${isMedia && !isCompleted ? "animate-pulse" : ""}`}>
          <Icon size={20} />
        </div>
...
export const Dashboard = ({ data, userId, createdAt, onReset, onExit, onStartInterview, onUpdate }: DashboardProps) => {
  ...
  const stats = useMemo(() => {
    ...
    safeArray<RoadmapPhase>(localData?.roadmap).forEach(phase => {
  ...
  const activePhase = useMemo(() => {
    const phases = safeArray<RoadmapPhase>(localData?.roadmap);
    const phase = phases[activePhaseIndex - 1];
    if (!phase) return null;
    return { ...phase, tasks: safeArray<RoadmapTask>((phase as any).tasks) } as RoadmapPhase & { tasks: RoadmapTask[] };
  }, [localData, activePhaseIndex]);
  ...
  const scheduleSave = useCallback(
    (nextData: AnalysisResult) => {
      ...
      saveTimerRef.current = window.setTimeout(() => {
        const doSave = async () => {
          try {
            await roadmapService.saveRoadmap(userId, (nextData as any).role_name, nextData);
          } catch (e) {
  ...
  const handlePlayResource = useCallback((phaseIdx: number, taskKey: string) => {
    const phase = safeArray<any>(localData?.roadmap)[phaseIdx];
    const tasks = safeArray<any>(phase?.tasks);
    const task = tasks.find((t: any, idx: number) => makeTaskKey(t, idx) === taskKey);
    if (task) setActiveResource(task);
  }, [localData]);
  ...
  const finishResource = useCallback(
    (rating?: number) => {
      ...
      if (taskIdx >= 0) {
        ...
        void handleTaskComplete(phaseIdx, taskKey, rating);
      }

      setActiveResource(null);
    },
    [activePhase, activeResource, activePhaseIndex, localData, handleTaskComplete]
  );
  ...
  const handleQuizPass = useCallback(() => {
    ...
    setActivePhaseIndex(prev => {
      const max = safeArray<any>(localData?.roadmap).length;
  ...
  const startDateDisp = createdAt ? formatUKShort(createdAt) : "Just now";

  const SidebarContent = () => (
    <>
      <div className="p-6 border-b border-gray-100 bg-white">
        <h2 className="text-xl font-black text-gray-900 mb-1 tracking-tight">Your Journey</h2>
        <p className="text-xs text-gray-500 font-medium uppercase tracking-widest truncate mb-6">
          {(localData as any)?.role_name || "Role"}
        </p>
          ...
          <div className="flex justify-between items-end relative z-10">
            <div>
              <div className="text-[10px] text-gray-400 font-bold uppercase mb-1">
                Level {1 + Math.floor(stats.currentXP / 1000)}
              </div>
        ...
        <VelocityWidget createdAt={createdAt || new Date().toISOString()} progress={stats.progress} totalWeeks={safeArray<any>(localData?.roadmap).length} />

        <div className="mt-4 flex items-center justify-between text-[10px] text-gray-400 font-medium px-1">
          <span className="flex items-center gap-1">
            <Flag size={10} /> Started {startDateDisp}
          </span>
          <span>{safeArray<any>(localData?.roadmap).length} Modules</span>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto no-scrollbar bg-gray-50/30">
        <RoadmapPath
          phases={safeArray<any>(localData?.roadmap)}
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-[#F8F9FB] font-sans overflow-hidden relative">
      ...
      <AnimatePresence>
        {quizSkill && <SkillQuiz skill={quizSkill} onClose={() => setQuizSkill(null)} onPass={handleQuizPass} />}
      </AnimatePresence>
      ...
      <main className="flex-1 h-full overflow-y-auto bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:24px_24px] scroll-smooth">
        <div className="max-w-4xl mx-auto p-6 md:p-12 pb-24">
          <motion.div
            key={(activePhase as any)?.label || activePhaseIndex}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mb-10"
          >
            <div className="flex items-center justify-between mb-6 md:hidden">
              <span className="text-xs font-bold uppercase tracking-widest text-gray-400">
                Phase {activePhaseIndex} of {safeArray<any>(localData?.roadmap).length}
              </span>
              <button
                type="button"
                onClick={() => setIsMobileMenuOpen(true)}
                className="p-2 bg-white rounded-lg border border-gray-200 shadow-sm text-gray-700"
              >
                <Menu size={20} />
              </button>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <div className="px-3 py-1 bg-white border border-gray-200 rounded-full text-xs font-bold text-gray-600 shadow-sm flex items-center gap-2">
                <Calendar size={12} />
                {activePhase.start_date ? formatUKShort(activePhase.start_date) : "Start"} to{" "}
                {activePhase.end_date ? formatUKShort(activePhase.end_date) : "End"}
              </div>

              <div className="px-3 py-1 bg-gray-100 rounded-full text-xs font-bold text-gray-400 flex items-center gap-2">
                <Timer size={12} /> Est {activePhase.total_hours || 0} Hours
              </div>
              ...
              {(() => {
                ...
                return (
                  <div
                    className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 border border-transparent ${status.bg} ${status.color} ${status.urgent ? "animate-pulse" : ""}`}
                  >
            ...
            <h1 className="text-4xl md:text-5xl font-black text-gray-900 mb-4 tracking-tight leading-tight">
              {(activePhase as any).focus_area}
            </h1>

            {renderDescription((activePhase as any).description || "")}
          </motion.div>
          ...
          <div className="grid grid-cols-1 gap-4">
            <AnimatePresence mode="popLayout">
              {safeArray<any>(activePhase?.tasks).map((task: any, tIdx: number) => {
                const done = isTaskDone(task);
                const key = makeTaskKey(task, tIdx);
                const phaseIdx = activePhaseIndex - 1;
                ...
                return (
                  <MissionCard
                    ...
                    task={task}
                    isCompleted={done}
                    onPlay={() => handlePlayResource(phaseIdx, key)}
                    onToggle={(rating?: number, metaPatch?: Record<string, any>) =>
                      void handleTaskComplete(phaseIdx, key, rating, metaPatch)
                    }
                  />
                );
              })}
          ...
          {allTasksDone && (
            ...
            >
              ...
              <div className="relative z-10">
                {activePhaseIndex === safeArray<any>(localData?.roadmap).length ? (
                  <>
                  ...
                  <>
                    ...
                    <p className="text-gray-400 mb-6 max-w-md mx-auto">
                      To unlock the next module, verify your knowledge of{" "}
                      <span className="text-white font-bold">{(activePhase as any).focus_area}</span>.
                    </p>
                    <button
                      type="button"
                      onClick={() => setQuizSkill((activePhase as any).focus_area)}
